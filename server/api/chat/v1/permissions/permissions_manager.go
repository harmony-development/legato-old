...
func (p *Manager) GetPermissions(forGuild, forChannel, forRole uint64) (ret []*chatv1.Permission) {
	guild := p.ensureGuild(forGuild)

	if forChannel == 0 {
		data := guild.Roles[RoleID(forRole)]
		for _, node := range data {
			ret = append(ret, &chatv1.Permission{
				Matches: node.Glob.s,
				Mode: func() chatv1.Permission_Mode {
					if node.Mode == Allow {
						return chatv1.Permission_Allow
					}
					return chatv1.Permission_Deny
				}(),
			})
		}
	} else {
...
func (p *Manager) SetPermissions(permissions []*chatv1.Permission, forGuild, forChannel, forRole uint64) error {
	guild := p.ensureGuild(forGuild)
	...
	for _, perm := range permissions {
		...
		if perm.Mode == chatv1.Permission_Allow {
			node.Mode = Allow
		} else {
			node.Mode = Deny
		}
